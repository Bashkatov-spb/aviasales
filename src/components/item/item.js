...
const Item = ({item}) => {
  ...
  const desTimeUp = format(new Date((+(new Date(segmentUp.date)))+(+(new Date(2022,0,0,hoursUp + 3,minutesUp)))), 'HH:mm');
  const depTimeDown = format(new Date(segmentDown.date), 'HH:mm');
  const desTimeDown = format(new Date((+(new Date(segmentDown.date)))+(+(new Date(2022,0,0,hoursDown + 3,minutesDown)))), 'HH:mm');
  ...
  return (
    <div className="items__item">
      <div className="items__item-header">
        ...
        <div style={{background: `url("https://pics.avs.io/99/36/${item.carrier}.png") no-repeat center`}} className="items__item-company"></div>
      </div>
        <div className="items__item-info">
          <div className="items__item-onewayticket">
            <div className="items__item-clock">
              <p className="items__item-up"><span>{segmentUp.origin}</span> - <span>{segmentUp.destination}</span></p>
              <p className="items__item-down"><span>{depTimeUp}</span> - <span>{desTimeUp}</span></p>
            </div>
            ...
            <div className="items__item-transfer">
              <p className="items__item-up">{segmentUp.stops.length} {stopsUp}</p>
              <p className="items__item-down">{transferUp}</p>
            </div>
          </div>
          <div className="items__item-twowayticket">
            <div className="items__item-clock">
              <p className="items__item-up"><span>{segmentDown.origin}</span> - <span>{segmentDown.destination}</span></p>
              <p className="items__item-down"><span>{depTimeDown}</span> - <span>{desTimeDown}</span></p>
            </div>
            <div className="items__item-duration">
              <p className="items__item-up">В пути</p>
            ...
            <div className="items__item-transfer">
              <p className="items__item-up">{segmentDown.stops.length} {stopsDown}</p>
